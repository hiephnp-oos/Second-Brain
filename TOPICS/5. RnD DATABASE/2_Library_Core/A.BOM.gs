 ...
 * * CHANGELOG:
 * - V4.3 - 28/05/26: [CLEANUP] Remove emojis from alerts/toasts, rename Map/Set variables per GAS anti AI slope rules.
 * - V4.2 - 13/04/26: [CRITICAL BUGFIX] Fix lỗi blank sheet do `setValues` crash vì dính `undefined`. Chuẩn hóa chuỗi Range Name bọc nháy đơn `'A.BOM'!A1` cho API. Gỡ Filter cũ trước khi clearContent để tránh kẹt lưới Grid.
 * - V4.1 - 13/04/26: [AUTO-REVISION] Thêm lệnh `rebuildSearchIndex()` ở cuối luồng để tự động nhảy Version.
 * - V4.0 - 13/04/26: [ZERO-LAG] Tối ưu hóa Data/Layout Separation. Thay `clear()` bằng `clearContent()`.
 * ======================================================================================
 */
...
function generateSmartBOM() {
  ...
  const BOM_SHEET_NAME = CONFIG.SHEETS.SYS_BOM; // "ZVNPPBOM"
  const MAT_SHEET_NAME = CONFIG.SHEETS.SYS_MAT; // "ZVNMMMAT"
  const TARGET_SHEET_NAME = CONFIG.SHEETS.BOM;  // "A.BOM"
  ...
  // ==================================================================================
  // PHẦN 1: LOGIC XỬ LÝ DỮ LIỆU (NGUYÊN BẢN)
  // ==================================================================================
  ...
  for (let i = 0; i < matData.length; i++) {
    const r = matData[i];
    const sapCode = String(r[0]).trim(); // Cột A
    const oldCode = String(r[3]).trim(); // Cột D
  ...
  for (let i = 0; i < bomData.length; i++) {
    ...
    if (!sapToOldCodeMap.has(compSap)) {
      badParentSapSet.add(String(row[0]).trim());
    }
  }
  ...
  for (let i = 0; i < bomData.length; i++) {
    ...
    if (badParentSapSet.has(parentSap)) {
      ghostCompCount++; continue;
    }
    ...
    if (!parentOld) {
      ghostParentCount++; continue;
    }
    
    let compOldNoAt = compOld;
    if (compOldNoAt.includes("@")) compOldNoAt = compOldNoAt.split("@")[0];

    // [FIX 1] Lọc sạch mọi nguy cơ undefined làm crash hàm setValues
    const cleanRow = [
      row[0],   // Material
      row[2],   // Material Description
      row[8],   // Base quantity
      row[9],   // Base unit of measure
      row[12],  // Item Number
      row[13],  // Component
      row[14],  // Component desc.
      row[16],  // Component quantity
      row[17],  // Component unit
      parentOld,// Material (old code)
      compOld,  // Component (old code)
      compOldNoAt // Component (old code) no @
  ...
  // ==================================================================================
  // PHẦN 2: RENDERING & GHI DỮ LIỆU (TỐI ƯU HÓA DATA/LAYOUT SEPARATION)
  // ==================================================================================
  ...
  const headers = [
    "Material", "Material Description", "Base quantity", "Base unit of measure", 
    "Item Number", "Component", "Component desc.", "Component quantity", 
    "Component unit", "Material (old code)", "Component (old code)", 
    "Component (old code) no @"
  ];
  ...
  // [FIX 2] Gỡ Filter cũ trước khi clear data để tránh lỗi lưới ẩn dòng
  if (targetSheet.getFilter()) {
    targetSheet.getFilter().remove();
  }

  if (!isNewSheet && targetSheet.getLastRow() > 0) {
    targetSheet.getDataRange().clearContent();
  }
  SpreadsheetApp.flush(); // Ép đồng bộ việc xóa dữ liệu trước khi ghi

  // [FIX 3] Bọc nháy đơn tên Sheet để Sheets API parse chính xác chuỗi "A.BOM"
  const rangeStr = `'${TARGET_SHEET_NAME}'!A1`;
  
  try {
    Sheets.Spreadsheets.Values.update({ values: allValues }, ss.getId(), rangeStr, {valueInputOption: "USER_ENTERED"});
  } catch (e) {
    console.warn("API Update failed, fallback to setValues: " + e.message);
    targetSheet.getRange(1, 1, totalRows, totalCols).setValues(allValues);
  }
  SpreadsheetApp.flush();

  // Format Batch (Chỉ chạy định dạng nếu là sheet mới tạo để tránh API vô ích)
  if (isNewSheet) {
    ...
    requests.push({
      "repeatCell": {
        "range": { "sheetId": sheetId, "startRowIndex": 1, "endRowIndex": totalRows },
        "cell": { "userEnteredFormat": { "numberFormat": { "type": "TEXT" }, "wrapStrategy": "CLIP" }},
    ...
    if (currentMaxCols > totalCols) {
      requests.push({
        "deleteDimension": {
          "range": { "sheetId": sheetId, "dimension": "COLUMNS", "startIndex": totalCols, "endIndex": currentMaxCols }
        }
      });
    }

    if (requests.length > 0) {
      try { Sheets.Spreadsheets.batchUpdate({ requests: requests }, ss.getId()); } catch(e) {}
    }
  }

  // ==================================================================================
  // PHẦN 3: BẢO VỆ SHEET & HOÀN TẤT
  // ==================================================================================
  
  applyStandardProtection(targetSheet, `Protected ${TARGET_SHEET_NAME}`, ['A1']);

  // Filter thông minh (Đã an toàn vì ở trên xóa rồi)
  if (!targetSheet.getFilter()) {
    targetSheet.getRange(1, 1, totalRows, totalCols).createFilter();
  }
  ...
  // Ép đổi DATA_VERSION để hủy Cache của SearchBOM
  if (typeof rebuildSearchIndex === 'function') rebuildSearchIndex();
    // --- BỔ SUNG CACHE WARMING Ở ĐÂY ---
  if (typeof warmUpBomIndex === 'function') {
    ss.toast("Dang nap du lieu BOM vao RAM Cache...", "Cache Warming", 2);
    warmUpBomIndex();
  }
}
