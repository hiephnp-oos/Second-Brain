...
| **SB-17** | **Dùng mutation atomic, không commit từng file.** Với thay đổi logic nhiều file, tuyệt đối không dùng `update_file` / `create_file` / `delete_file` tuần tự trực tiếp trên `main`. Hãy build toàn bộ target tree, tạo một commit duy nhất rồi mới update `main`. Nếu interface không hỗ trợ atomic commit, dùng temporary branch/worktree và chỉ publish final state đã validate. Single-file low-risk change không có dependency mới có thể dùng direct file update. | Mọi thay đổi nhiều file / rename / move / cleanup / migration |
